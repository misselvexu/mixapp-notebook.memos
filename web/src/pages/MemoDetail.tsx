        ...
        <div className={cn(md ? "w-[calc(100%-15rem)]" : "w-full")}>
          {parentMemo && (
            <div className="w-auto inline-block mb-2">
              <Link
                className="px-3 py-1 border border-[var(--color-border)] rounded-lg max-w-xs w-auto text-sm flex flex-row justify-start items-center flex-nowrap text-[var(--color-muted-foreground)] dark:border-[var(--color-border)] hover:shadow hover:opacity-80"
                to={`/${parentMemo.name}`}
                state={locationState}
                viewTransition
              >
                ...
                showCreateCommentButton && (
                  <div className="w-full flex flex-row justify-center items-center py-6">
                    <Button variant="ghost" onClick={handleShowCommentEditor}>
                      <span className="text-[var(--color-muted-foreground)]">{t("memo.comment.write-a-comment")}</span>
                      <MessageCircleIcon className="ml-2 w-5 h-auto text-[var(--color-muted-foreground)]" />
                    </Button>
                  </div>
                )
              ) : (
                <>
                  <div className="w-full flex flex-row justify-between items-center h-8 pl-3 mb-2">
                    <div className="flex flex-row justify-start items-center">
                      <MessageCircleIcon className="w-5 h-auto text-[var(--color-muted-foreground)] mr-1" />
                      <span className="text-[var(--color-muted-foreground)] text-sm">{t("memo.comment.self")}</span>
                      <span className="text-[var(--color-muted-foreground)] text-sm ml-1">({comments.length})</span>
                    </div>
                    {showCreateCommentButton && (
                      <Button variant="ghost" className="text-[var(--color-muted-foreground)]" onClick={handleShowCommentEditor}>
