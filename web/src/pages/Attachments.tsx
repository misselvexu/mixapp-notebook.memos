  ...
  return (
    <section className="@container w-full max-w-5xl min-h-full flex flex-col justify-start items-center sm:pt-3 md:pt-6 pb-8">
      ...
      <div className="w-full px-4 sm:px-6">
        <div className="w-full shadow flex flex-col justify-start items-start px-4 py-3 rounded-xl bg-[var(--color-card)] text-[var(--color-card-foreground)]">
          <div className="relative w-full flex flex-row justify-between items-center">
            ...
            <div>
              <div className="relative max-w-32">
                <SearchIcon className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-[var(--color-muted-foreground)]" />
                <Input
                  className="pl-9"
                            ...
                            {attachments.map((attachment) => {
                              return (
                                <div key={attachment.name} className="w-24 sm:w-32 h-auto flex flex-col justify-start items-start">
                                  <div className="w-24 h-24 flex justify-center items-center sm:w-32 sm:h-32 border border-[var(--color-border)] overflow-clip rounded-xl cursor-pointer hover:shadow hover:opacity-80">
                                  ...
                                  <div className="w-full max-w-full flex flex-row justify-between items-center mt-1 px-1">
                                    <p className="text-xs shrink text-[var(--color-muted-foreground)] truncate">{attachment.filename}</p>
                                  </div>
                                </div>
                              );
                            })}
                        ...
                        <div className="w-full flex flex-row justify-start items-start">
                          ...
                          <div className="w-full max-w-[calc(100%-4rem)] sm:max-w-[calc(100%-6rem)] flex flex-row justify-start items-start gap-4 flex-wrap">
                            <div className="w-full flex flex-row justify-start items-center gap-2">
                              <span className="text-[var(--color-muted-foreground)]">{t("resource.unused-resources")}</span>
                              <span className="text-[var(--color-muted-foreground)] opacity-80">({unusedAttachments.length})</span>
                              <TooltipProvider>
                                <Tooltip>
