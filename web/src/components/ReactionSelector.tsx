  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <span
          className={cn(
            "h-7 w-7 flex justify-center items-center rounded-full border border-[var(--color-border)] hover:opacity-70 cursor-pointer",
            className,
          )}
        >
          <SmilePlusIcon className="w-4 h-4 mx-auto text-[var(--color-muted-foreground)]" />
        </span>
      </PopoverTrigger>
            ...
            {workspaceMemoRelatedSetting.reactions.map((reactionType) => {
              return (
                <span
                  key={reactionType}
                  className={cn(
                    "inline-flex w-auto text-base cursor-pointer rounded px-1 text-[var(--color-muted-foreground)] hover:opacity-80",
                    hasReacted(reactionType) && "bg-blue-100 dark:bg-[var(--color-card)]",
                  )}
