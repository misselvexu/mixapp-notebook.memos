...
const getCellOpacity = (ratio: number): string => {
  if (ratio === 0) return "";
  if (ratio > 0.75) return "bg-green-700/90 text-[var(--color-muted-foreground)]/80";
  if (ratio > 0.5) return "bg-green-700/70 text-[var(--color-muted-foreground)]/60";
  if (ratio > 0.25) return "bg-green-700/70 text-[var(--color-muted-foreground)]/40";
  return "bg-green-700/50 text-[var(--color-muted-foreground)]/20";
};
  ...
  ({
    ...
    const cellContent = (
      <div
        className={cn(
          ...
          "rounded-lg border-2 text-[var(--color-muted-foreground)] transition-all duration-200",
          dayInfo.isCurrentMonth && getCellOpacity(count / maxCount),
          dayInfo.isCurrentMonth && isToday && "border-[var(--color-border)]",
          dayInfo.isCurrentMonth && isSelected && "font-medium border-[var(--color-border)]",
    ...
    if (!dayInfo.isCurrentMonth) {
      return (
        <div
          className={cn(
            "w-6 h-6 text-xs lg:text-[13px] flex justify-center items-center cursor-default opacity-60 text-[var(--color-muted-foreground)]",
          )}
        >
          {dayInfo.day}
        </div>
