...
const MemoRelationListView = (props: Props) => {
  ...
  return (
    <div className="relative flex flex-col justify-start items-start w-full px-2 pt-2 pb-1.5 bg-zinc-50 dark:bg-[var(--color-card)] rounded-lg border border-[var(--color-border)]">
      <div className="w-full flex flex-row justify-start items-center mb-1 gap-3 opacity-60">
        {referencingMemoList.length > 0 && (
          <button
            className={cn(
              "w-auto flex flex-row justify-start items-center text-xs gap-0.5 text-[var(--color-muted-foreground)]",
              selectedTab === "referencing" && "text-[var(--color-foreground)]",
            )}
        ...
        {referencedMemoList.length > 0 && (
          <button
            className={cn(
              "w-auto flex flex-row justify-start items-center text-xs gap-0.5 text-[var(--color-muted-foreground)]",
              selectedTab === "referenced" && "text-[var(--color-foreground)]",
            )}
        ...
        <div className="w-full flex flex-col justify-start items-start">
          {referencingMemoList.map((memo) => {
            return (
              <Link
                key={memo.name}
                className="w-auto max-w-full flex flex-row justify-start items-center text-sm leading-5 text-[var(--color-muted-foreground)] dark:border-[var(--color-border)] hover:underline"
                to={`/${memo.name}`}
                viewTransition
                state={{
                  from: parentPage,
              ...
              >
                <span className="text-xs opacity-60 leading-4 border border-[var(--color-border)] font-mono px-1 rounded-full mr-1 dark:border-[var(--color-border)]">
