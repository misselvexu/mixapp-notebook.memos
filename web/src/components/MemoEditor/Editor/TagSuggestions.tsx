...
const TagSuggestions = observer(({ editorRef, editorActions }: Props) => {
  ...
  return (
    <div
      className="z-20 p-1 mt-1 -ml-2 absolute max-w-48 gap-px rounded font-mono flex flex-col justify-start items-start overflow-auto shadow bg-[var(--color-secondary)]"
    ...
    >
      {suggestionsRef.current.map((tag, i) => (
        <div
          ...
          className={cn(
            "rounded p-1 px-2 w-full truncate text-sm dark:text-[var(--color-muted-foreground)] cursor-pointer hover:bg-zinc-200 dark:hover:bg-[var(--color-card)]",
            i === selected ? "bg-[var(--color-secondary)]" : "",
          )}
        >
