  ...
  const MediaCard = ({ attachment, className }: { attachment: Attachment; className?: string }) => {
    ...
    if (type === "image/*") {
      return (
        <img
          className={cn(
            "cursor-pointer h-full w-auto rounded-lg border border-[var(--color-border)] object-contain hover:opacity-80",
            className,
          )}
      ...
      return (
        <video
          className={cn(
            "cursor-pointer h-full w-auto rounded-lg border border-[var(--color-border)] object-contain bg-[var(--color-secondary)]",
            className,
          )}
          preload="metadata"
