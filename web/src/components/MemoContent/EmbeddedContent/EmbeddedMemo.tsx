...
const EmbeddedMemo = observer(({ resourceId: uid, params: paramsStr }: Props) => {
  ...
  const contentNode = useSnippet ? (
    <div className={cn("text-[var(--color-foreground)]", inlineMode ? "" : "line-clamp-3")}>{memo.snippet}</div>
  ) : (
    <>
      <MemoContent
  ...
  return (
    <div className="relative flex flex-col justify-start items-start w-full px-3 py-2 bg-zinc-50 dark:bg-[var(--color-card)] rounded-lg border border-[var(--color-border)] hover:shadow">
      <div className="w-full mb-1 flex flex-row justify-between items-center text-[var(--color-muted-foreground)]">
