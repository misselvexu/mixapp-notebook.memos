    ...
    <div className="w-full mt-2 flex flex-row justify-start items-center flex-wrap gap-x-2 gap-y-1">
      {filters.map((filter: MemoFilter) => (
        <div
          ...
          className="w-auto leading-7 h-7 shrink-0 flex flex-row items-center gap-1 bg-white dark:bg-[var(--color-card)] border border-[var(--color-border)] pl-1.5 pr-1 rounded-md hover:line-through cursor-pointer"
          onClick={() => memoFilterStore.removeFilter((f: MemoFilter) => isEqual(f, filter))}
        >
          <FactorIcon className="w-4 h-auto text-[var(--color-muted-foreground)] opacity-60" factor={filter.factor} />
          <span className="text-[var(--color-muted-foreground)] text-sm max-w-32 truncate">{getFilterDisplayText(filter)}</span>
          <button className="text-[var(--color-muted-foreground)] opacity-60 hover:opacity-100">
